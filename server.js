...
app.get("/api/swiggy/restaurantMenu", async (req, res) => {
    try {
        const {resId}=req.query
        const response = await axios.get(`https://www.swiggy.com/dapi/menu/pl?page-type=REGULAR_MENU&complete-menu=true&lat=12.9715987&lng=77.5945627&restaurantId=${resId}&catalog_qa=undefined&query=Biryani&submitAction=ENTER`, {
            headers: { "User-Agent": "Mozilla/5.0" } // Mimic a browser request
        });
        res.json(response.data); //here we are sending data inside the response. Then why we are again looking for data in Body.js?
    } catch (error) {
